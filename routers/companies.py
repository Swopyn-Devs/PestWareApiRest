...
async def show(company_id: UUID4, db: Session = Depends(get_db), token: str = Depends(oauth2_schema)):
...
async def update(company_id: UUID4, request: CompanyRequest, db: Session = Depends(get_db), token: str = Depends(oauth2_schema)):
...
async def update_document_logo(company_id: UUID4, file: UploadFile, db: Session = Depends(get_db), token: str = Depends(oauth2_schema)):
...
async def update_document_stamp(company_id: UUID4, file: UploadFile, db: Session = Depends(get_db), token: str = Depends(oauth2_schema)):
...
async def update_web_logo(company_id: UUID4, file: UploadFile, db: Session = Depends(get_db), token: str = Depends(oauth2_schema)):
    return company.update_web_logo(db, file, company_id)


@router.delete('/{company_id}', status_code=status.HTTP_204_NO_CONTENT)
async def destroy(company_id: UUID4, db: Session = Depends(get_db), token: str = Depends(oauth2_schema)):
