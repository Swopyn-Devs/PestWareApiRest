...
async def show(job_title_id: UUID4, db: Session = Depends(get_db)):
    return job_title.retrieve(db, job_title_id)


@router.post('', status_code=status.HTTP_201_CREATED, response_model=JobTitleResponse)
async def store(request: JobTitleRequest, db: Session = Depends(get_db)):
...
async def destroy(job_title_id: UUID4, db: Session = Depends(get_db)):
