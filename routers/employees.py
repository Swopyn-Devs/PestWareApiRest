...
async def show(employee_id: UUID4, db: Session = Depends(get_db), token: str = Depends(oauth2_schema)):
    return employee.retrieve(db, employee_id)


@router.post('', status_code=status.HTTP_201_CREATED, response_model=EmployeeResponse)
async def store(request: EmployeeRequest, db: Session = Depends(get_db), token: str = Depends(oauth2_schema)):
...
async def destroy(employee_id: UUID4, db: Session = Depends(get_db), token: str = Depends(oauth2_schema)):
