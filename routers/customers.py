...
@router.get('/csv/{customer_id}', status_code=status.HTTP_200_OK)
...
async def faker(count: int = Query(1), db: Session = Depends(get_db), authorize: AuthJWT = Depends()):
    authorize.jwt_required()
    return customer.faker(db, count)
