...
from documentation.business_activities import *
from schemas.job_center import JobCenterResponse, JobCenterBasicResponse


class BusinessActivityRequest(BaseModel):
...
class BusinessActivityUpdateRequest(BaseModel):
    name: str = Field(title=title_name, description=desc_name, max_length=255, min_length=3, example=ex_name)
...
class BusinessActivityResponse(BaseModel):
...
class BusinessActivityBasicResponse(BaseModel):
    id: UUID4 = Field(title=title_id, description=desc_id, example=ex_id)
    name: str = Field(title=title_name, description=desc_name, max_length=255, min_length=3, example=ex_name)
    job_center_id: UUID4
