...
class CustomerPortalAccountRequestUpdated(BaseModel):
    ...
    is_active: Optional[bool] = Field(title='Estatus', description='Activar o inactivar cuenta para bloquear el login.', example=True)
...
class SendCredentialsRequest(BaseModel):
    email: EmailStr = Field(name='Correo del usuario propietario de la cuenta.', example='example@swopyn.com')
