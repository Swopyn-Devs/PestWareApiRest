...
def response_customer(db, customer, basic=False, primary=True):
    ...
    if type(customer) is dict:
        ...
        return CustomerResponse(
            ...
            is_main=customer.is_main,
            main_customer_id=customer.main_customer_id,
            job_center_id=functions.get_data(db, JobCenter, customer.job_center_id, 'centro de trabajo', False, False, True, True, 1),
            business_activity_id=functions.get_data(db, BusinessActivity, customer.business_activity_id, 'giro de la empresa', False, False, True, True, 1),
            total_branches=get_total_branches(db, customer.id),
            total_quotes=23,
            total_scheduled_services=68,
            total_completed_services=140,
